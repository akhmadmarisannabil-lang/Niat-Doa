...
class AnNasrPage extends StatefulWidget {
  final Map<String, dynamic> surahData;

  const AnNasrPage({super.key, required this.surahData});

  @override
  State<AnNasrPage> createState() => _AnNasrPageState();
}

class _AnNasrPageState extends State<AnNasrPage> {
  final AudioPlayer player = AudioPlayer();
  bool isPlayingFullSurah = false;

  final ScrollController _scrollController = ScrollController();
  final List<GlobalKey> ayatKeys = List.generate(3, (_) => GlobalKey());
  ...
  Future<void> loadAudioData() async {
    final data = await QuranApiService.getSurat(110);
    ...
    final List<Map<String, dynamic>> ayatList = [
      {
        ...
        'transliteration': "Iżā jā'a naṣrullāhi wal-fatḥ(u).",
      ...
      },
      {
        'no': '3',
        'arabic':
            "فَسَبِّحْ بِحَمْدِ رَبِّكَ وَاسْتَغْفِرْهُۗ اِنَّهٗ كَانَ تَوَّابًاࣖ",
        'transliteration':
            "Fa sabbiḥ biḥamdi rabbika wastagfirh(u), innahū kāna tawwābā(n).",
        'translation':
            "bertasbihlah dengan memuji Tuhanmu dan mohonlah ampunan kepada-Nya. Sesungguhnya Dia Maha Penerima tobat.",
      },
    ];
    return Scaffold(
        ...
        children: [
          Wrap(
            ...
            children: [
              ...
              _buildChip(
                "${widget.surahData['verses'] ?? 3} Ayat",
                chipBgColor,
                chipTextColor,
              ),
